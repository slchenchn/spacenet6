...
def customsoftmax(inp, multihotmask, eps=1e-6):
    """
    Custom Softmax
    """
    soft = F.softmax(inp)
    ...
    tmp = (soft * multihotmask).sum(1, keepdim=True)
    tmp = multihotmask * tmp
    tmp = torch.max(soft, tmp)
    # if (tmp <= 0).any():
    #     print('breakpoint')
    # NOTE: 这里需要将小于eps的置为eps，否则梯度会变成nan
    tmp[tmp<=eps] = eps
    res = torch.log(tmp)
    # res.sum().backward()
    return res
...
class ImgWtLossSoftNLL(nn.Module):
    ...
    def custom_nll(self, inputs, target, class_weights, border_weights, mask):
        ...
        if self.fp16:
            ...
            tmp = customsoftmax(inputs, target[:, :-1, :, :].float())
            tmp1 = -1 / border_weights * (target[:, :-1, :, :].float() *
                            class_weights.unsqueeze(0).unsqueeze(2).unsqueeze(3) * tmp).sum(1)
            loss_matrix = tmp1 * (1. - mask.float())

            # loss_matrix[border_weights > 1] = 0
        # TODO: 这里直接将nan和inf置零0，没有探索其原因
        loss_matrix[loss_matrix.isnan()] = 0
        loss_matrix[loss_matrix.isinf()] = 0
    ...
    def forward(self, inputs, target):
        ...
        for i in range(0, inputs.shape[0]):
            if not self.batch_weights:
                ...
                # NOTE: cancel class balance
                class_weights = np.ones(2, )
            loss += self.custom_nll(inputs[i].unsqueeze(0),
...
class JointEdgeSegLoss(nn.Module):
    def __init__(self,
        ...
        self.seg_body_loss = ImgWtLossSoftNLL(classes=num_classes,
                                     ignore_index=ignore_index,
                                     upper_bound=1.0, ohem=False).cuda()
        # self.edge_ohem_loss = OhemCrossEntropy2dTensor(ignore_index=ignore_index, min_kept=5000).cuda()
        self.edge_ohem_loss = CrossEntropyLoss2d(ignore_index=ignore_index, size_average=True).cuda()
    ...
    def edge_attention(self, input, target, edge):
        ...
        edge_ohem_loss = self.edge_ohem_loss(input, torch.where(edge.max(1)[0] > 0.8, target, filler))
        
        # TODO: 这里直接将nan和inf置零0，没有探索其原因
        if torch.isnan(edge_ohem_loss):
            edge_ohem_loss = 0
        return edge_ohem_loss

    def forward(self, inputs, targets, weight=None, ignore_index=255):
        # separate data
        seg_in, seg_body_in, edge_in = inputs
        seg_bord_mask = targets[:, :-1, ...]
        edgemask = targets[:, -1, ...].unsqueeze(1).float()

        # if targets.ndim < 4:
        #     print('breakpoint')

        segmask = onehot2label(seg_bord_mask, ignore_index=self.ignore_index)
        losses = {}

        losses['seg'] = self.loss_weight * self.seg_weight * self.seg_loss(seg_in, segmask)
        losses['body'] = self.loss_weight * self.seg_body_weight * self.seg_body_loss(seg_body_in, seg_bord_mask)
        losses['edge'] = self.loss_weight * self.edge_weight * 20 * self.bce2d(edge_in, edgemask)
        # losses['edge_ohem_loss'] = self.att_weight * self.edge_attention(seg_in, segmask, edge_in)
        # logger.info(f'seg_loss: {losses["seg_loss"]}, seg_body: {losses["seg_body"]}'
        #             f', edge_loss: {losses["edge_loss"]}')
...
def onehot2label(target, ignore_index=255):
    """
    Args:
        target:
    ...
    """
    label = torch.argmax(target[:, :-1, :, :], dim=1).long()
    label[target[:, -1, :, :].bool()] = ignore_index
    return label